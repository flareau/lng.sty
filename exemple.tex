...
\begin{itemize}
	\item Signe linguistique: \sign{pleuvait} = \sign{pleuv--}\comb\sign{--ait}
	\item Graphèmes: \graphm{pluie}
	\item Sens: \sem{pluie}
	\item Concept: \conc{pluie}
	\item Lexie: \lex{pleuvoir}
	\item Grammème: \lex{pleuvoir}.\gramm{imp}.\gramm{3sg}
	\item Locution: \idiom{\lex{tomber des cordes}}, \sign{Il \idiom{tombait des cordes}}
	...
	\item Relations étiquetées: \sign{Il \lrel{suj} pleuvait \rrel{mod} souvent}
	\item Énoncé agrammatical: \ungr\sign{Il n'a pas le fait}
	\item Énoncé douteux: \doubt\sign{Il s'est en allé}
	\item Énoncé sémantiquement bizarre: \infel\sign{Tu manges une patate très douce}
	\item Formulations alternatives: \sign{Il pleuvait\slashup neigeait\slashup grêlait}
\end{itemize}


\section{Exemples linguistiques}

\ex. \label{ex:silence}\sign{Il règne un très lourd silence dans la pièce.} \hfill(source)\\
...
\avm[style=plain]{
	[ pdd & N \\
	  sens & \sem{partir} \\
	  genre & \gramm{masc} \\
	  tr & [ \rel{I} & [ arg & \rel{1} \\ pdd & N \\ prép & \lex{de}]]]
}
